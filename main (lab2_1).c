...
TRISA4=1;
TRISA|=0b00101111; //перевод выводов RA0, RA1, RA2, RA3, RA5 в режим
TRISE|=0b00000111; //перевод выводов RE0, RE1, RE2 в режим входов
    ...
    while(1)
        ...
        A=PORTAbits.RA4;  
        inttolcd(0x80, i);
            inttolcd(0xC0, read_Adc(i));
            __delay_ms(100);
        /*switch (i){
            case 0:    
            ADCON0bits.CHS=0; // выбор аналогового канала
ADCON0bits.GO_DONE=1; // запуск преобразования
while(ADCON0bits.GO_DONE==1);
return (ADRESH<<2)+(ADRESL>>6);//запись результата преобразования
            break; 
...
ADCON0bits.GO_DONE=1; // запуск преобразования
while(ADCON0bits.GO_DONE==1);
return (ADRESH<<2)+(ADRESL>>6);//запись результата преобразованияeak;
            case 7:    
            ADCON0bits.CHS=0; // выбор аналогового канала
ADCON0bits.GO_DONE=1; // запуск преобразования
while(ADCON0bits.GO_DONE==1);
