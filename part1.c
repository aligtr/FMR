...
#ifndef _XTAL_FREQ
#define _XTAL_FREQ 40000000 // определение тактовой частоты  40 ћ√ц  дл€ функции __delay_ms
#endif
...
void lcd_clk(void) /*генераци€ импульса на вход EN*/
{
  LCD_E = 1;
...
void lcd_putc(char outbyte) /* отправить данные (4-битна€ операци€) */
{
...
void lcd_init() // инициализаци€ LCD-диспле€
{
...
/*---------------------------------*/
  ...
  lcd_command(0x01);	// очистить всЄ
  lcd_command(0x06);	// автоматически передвинуть курсор после байта
  lcd_command(0x0C);	// дисплей включЄн, курсора нет, не моргает
  lcd_command(0x02);	// начальна€ позици€
  lcd_command(0x01);	// очистить всЄ снова
}
